...
- テックリードとプロジェクトマネージャーの兼任:
  両役割の要求が相反する場面がありました。  
  特に、AWS での構築を進めていた際、経営層の期待するランニングコストの 2 倍かかることが判明し、技術的決定とプロジェクト管理の観点で調整が必要となりました。  
  Cloudflare への移行を検討し、コスト削減は可能だがプロジェクト完了が 2 週間遅延する見込みとなり、この判断に苦慮しました。経営層との綿密な協議を重ね、長期的なコスト削減の重要性を共有し、技術的な最適解を優先する決断を下しました。

- 未知のプロトコル（OCPP）への対応:
  充電器管理の国際標準プロトコルである OCPP は複雑で、チーム全体での理解に苦労しました。  
  この課題に対し、チーム内で仕様書の読み合わせセッションを定期的に実施し、解釈の共通認識を形成しました。  
  さらに、これらの理解をドキュメントとして残すことで、知識の共有と継続的な参照を可能にし、結果としてスムーズな開発環境を構築することができました。
...
- 充電器管理の国際標準プロトコルである"OCPP"に対応できていない
- 長年の改修により、継ぎ足し継ぎ足しのコードになっており、新たな機能追加が非常にしづらい
- Typescript で作ってはいるが、型情報がほとんどなくコードリーディングがしづらい
...
- 兼任によるメリットと課題への対応
  兼任することで、技術的な深い理解をプロジェクト管理に直接活かすことができ、迅速な意思決定と柔軟な対応が可能となりました。一方で、役割間で相反する要求が生じた際には、プロジェクトの本質的な目的を常に念頭に置き、経営層とも綿密に協議しながら、技術的な最適解を優先する判断を行いました。
...
- 管理コストの大幅削減：
  元々のサーバー管理コストから約 1/3 程度にコストを抑えることができました。これにより、運用効率が大幅に向上し、リソースを他の重要な業務に振り分けることが可能になりました。

- 新プロトコル（OCPP）への対応完了：
  充電器管理の国際標準プロトコルである OCPP への対応が完了しました。これにより、ビジネスをよりスケールしていくための基盤が整い、国際展開や他社システムとの連携がスムーズになりました。

- コードベースの改善：
  機能の見通しが良く、保守性・拡張性の高いコードベースに改修することができました。  
  特に、TypeScript ベースに移行したことで型情報が加わり、開発効率の向上とバグの早期発見が可能になりました。これにより、今後の機能追加や改修作業がより効率的に行えるようになりました。

これらの成果により、システムの運用コストを削減しつつ、ビジネスの拡大と技術的な基盤の強化を同時に達成することができました。新しいシステムは、今後の事業成長を支える重要な資産となることが期待されます。
...
- クラウドインフラの最適化スキル：
  AWS から Cloudflare への移行を検討・実行する中で、クラウドインフラの選択がコストとパフォーマンスに与える影響を深く理解しました。これにより、将来のプロジェクトでより効果的なインフラ選択ができるようになりました。

- 経営層とのコミュニケーション能力：  
  技術的な決定と経営的な判断の間でバランスを取る必要がある場面で、経営層との効果的なコミュニケーション能力が向上しました。技術的な内容を非技術者にも理解しやすく説明するスキルが磨かれました。

これらの学びにより、技術リーダーシップとプロジェクト管理の両面で大きく成長することができました。この経験は、今後のキャリアにおいて、より複雑で大規模なプロジェクトを成功に導くための貴重な基盤となるでしょう。
