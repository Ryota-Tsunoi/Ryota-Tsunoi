...
### No5. EV 充電器メーカーでの IoT デバイス管理用システム構築
...
EV 充電器には、充電器の状態を監視するための管理サーバーが必要であり、そのサーバー経由で遠隔での充電開始や停止などを制御できるようになっています。

元々稼働しているサーバーが存在していたのですが、次の課題を抱えていました。

- 充電器監視の国際標準プロトコルである"OCPP"に対応できていない
- 長年の改修により、継ぎ足し継ぎ足しのコードになっており、新たな機能追加が非常にしづらい
- TypeScript で作ってはいるが、型情報がほとんどなくコードリーディングしづらいコード
- オンプレミス運用で、サーバー管理のコストが高い

これらの課題を解決し、ビジネスをスケールしやすくするために、管理システムのリプレイスを決定しました。

#### 担当業務と役割

本プロジェクトでは、4 人という比較的小規模なチームで取り組みました。その中で私は、テックリードとプロジェクトマネージャー（PM）の 2 つの役割を兼任しました。  
これにより、技術的な深い理解とプロジェクト全体の管理を両立させ、効率的なプロジェクト運営を実現しました。

- テックリードとしての役割

  - 技術選定や重要な技術的決定の最終判断
  - インフラ、フロントエンド、バックエンド、DB など、開発全般への幅広い関与
  - エンジニアへの技術的サポートと指導
  - コードレビューと品質管理
...
- 兼任によるメリットと課題への対応:  
  兼任することで、技術的な深い理解をプロジェクト管理に直接活かすことができ、迅速な意思決定と柔軟な対応が可能となりました。  
  一方で、役割間で相反する要求が生じた際には、プロジェクトの本質的な目的を常に念頭に置き、経営層とも綿密に協議しながら、技術的な最適解を優先する判断をしました。
...
- バックエンド
  - Hono
- インフラ
  - Cloudflare Workers(バックエンドのデプロイ先として)
  - Cloudflare Pages(フロントエンドのデプロイ先として)
...
- テックリードとプロジェクトマネージャーの兼任:  
  両役割の要求が相反する場面もありました。 特に、AWS での構築を進めていた際、経営層の期待するランニングコストの 2 倍かかることが判明し、技術的決定とプロジェクト管理の観点で調整が必要となりました。  
  Cloudflare への移行を検討し、コスト削減は可能だがプロジェクト完了が 2 週間遅延する見込みとなり、この判断に苦慮しました。経営層との綿密な協議を重ね、長期的なコスト削減の重要性を共有し、技術的な最適解を優先する決断を下しました。

- 未知のプロトコル（OCPP）への対応:  
  充電器監視の国際標準プロトコルである OCPP は複雑で、チーム全体での理解に苦労しました。 この課題に対し、チーム内で仕様書の読み合わせセッションを定期的に実施し、解釈の共通認識を形成しました。 さらに、これらの理解をドキュメントとして残すことで、知識の共有と継続的な参照を可能にし、結果としてスムーズな開発環境を構築できました。

これらの課題に対して、技術的な深い理解とプロジェクト全体の視点を組み合わせたアプローチを採用することで、効果的な解決策を見出し、プロジェクトを成功に導くことができました。

#### プロジェクトの成果

システム開発の結果、次のような成果が得られました。

- 管理コストの大幅削減： 元々のサーバー管理コストから約 1/3 程度にコストを抑えることができました。これにより、運用効率が大幅に向上し、リソースを他の重要な業務に振り分けることが可能になりました。

- 新プロトコル（OCPP）への対応完了： 充電器監視の国際標準プロトコルである OCPP への対応が完了しました。これにより、ビジネスをよりスケールしていくための基盤が整い、国際展開や他社システムとの連携がスムーズになりました。

- コードベースの改善： 機能の見通しが良く、保守性・拡張性の高いコードベースに改修できました。 特に、TypeScript ベースに移行したことで型情報が加わり、開発効率の向上とバグの早期発見が可能になりました。これにより、今後の機能追加や改修作業がより効率的に行えるようになりました。

これらの成果により、システムの運用コストを削減しつつ、ビジネスの拡大と技術的な基盤の強化を同時に達成できました。新しいシステムは、今後の事業成長を支える重要な資産となることを期待しています。
...
- テックリードと PM の役割バランス：  
  両役割を兼任することで、技術的な深い理解とプロジェクト全体の俯瞰的な視点を統合することの重要性を学びました。特に、技術的な決定とビジネス要求のバランスを取る能力が向上しました。

- 複雑なプロトコルへの対応能力：  
  OCPP という未知のプロトコルに対応する過程で、複雑な仕様を理解し、チーム全体で知識を共有する効果的な方法を学びました。これにより、今後新しい技術に直面した際の学習と適応能力が強化されました。

- クラウドインフラの最適化スキル： AWS から Cloudflare への移行を検討・実行する中で、クラウドインフラの選択がコストとパフォーマンスに与える影響を深く理解しました。これにより、将来のプロジェクトでより効果的なインフラ選択ができるようになりました。

- 経営層とのコミュニケーション能力：  
  技術的な決定と経営的な判断の間でバランスを取る必要がある場面で、経営層との効果的なコミュニケーション能力が向上しました。技術的な内容を非技術者にも理解しやすく説明するスキルが磨かれました。

これらの学びにより、技術リーダーシップとプロジェクト管理の両面で大きく成長できました。この経験は、今後のキャリアにおいて、より複雑で大規模なプロジェクトを成功に導くための貴重な基盤となると考えています。

---

## 自己 PR

私の強みは、技術力とコミュニケーション能力を活かして、チームとともに高品質なプロダクトを開発することです。
フルスタック開発の経験から、システム全体を俯瞰しながら、ユーザーにとって価値あるソリューションを提供することを常に心がけています。
特に、レガシーシステムのモダナイゼーションやアジャイル開発の経験から、変化に柔軟に適応し継続的な改善を通じて、ビジネスの成長に貢献できると自負しています。

また、私はユーザー視点でのものづくりに情熱を注いでいます。
ユーザーの課題を深く理解し、それを解決することにやりがいを感じます。
要件定義の段階から、ユーザー部門との密なコミュニケーションを通じて、真のニーズを引き出し、それをシステムに反映させることを大切にしています。

さらに、私はチームワークを重視しています。
アジャイル開発のスクラムマスターとしての経験から、チームメンバーの強みを引き出し、心理的安全性の高い環境を作ることで、チームのパフォーマンスを最大化できると信じています。
技術的な議論だけでなく、メンバーの思いに寄り添い、ともに成長していける関係を築くことを大切にしています。

技術の進歩とビジネス環境の変化が速い IT 業界において、私は常に学び続け、新しい価値を生み出していきたいと考えています。
貴社の理念とビジョンに共感し、私の技術力とチーム構築力を存分に発揮することで、ビジネスの発展に貢献していきたいと思います。
