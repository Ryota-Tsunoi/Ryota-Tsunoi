...
Web ページの ScreenShot に、次の課題を感じており、それを解決するプロダクトを作ろうと考えました。
Enlarge 合同会社を立ち上げて初めてのプロダクト開発です。

- モバイル端末で、縦に長い Web ページ全体の ScreenShot を取得する手段がない
- Web ページの一部分だけを ScreenShot として取るために、いちいち"写真"アプリ等で編集する必要がある
- ScreenShot を"写真"フォルダに保存しても、その元となる Web ページの URL がどこかわからない

#### 担当業務と役割

- 要件定義から運用/保守、その後のアップデートまでアプリ開発に関わる様々なことを担当しました。
- アジャイルを基本とし、出来上がった部分から小さくリリースし、ユーザーからの反応を元に改善を加えていきました。

#### 技術スタック

- モバイルアプリ: Swift (開発当初は Flutter で進めていましたが、パフォーマンスや機能面で課題が出てきたため、Native での開発に切り替えました)
- バックエンド: Firebase を扱い、できる限り自分たちで管理するものを少なくしました。

#### プロジェクトの課題と解決策

- Flutter では実現の難しい機能があったため、Swift への移行を決断：Flutter のみでは実装の困難な機能があると判明したため、Swift を用いたネイティブコードでの開発にシフトする決断をしました。  
  また、スクリーンショット取得の速度を比較した結果、Swift 版の方が優れていたことも移行の決め手となりました。
- メモリリークの問題が発生し、安定性に課題：長い Web ページ全体のスクリーンショットを取得する際、Flutter 版ではメモリリークが頻発する問題に直面しました。  
  アプリの安定性を確保するためにも、この問題を解決する必要がありました。Swift への移行により、メモリ管理の問題を解消できました。
- モバイル開発の知識不足により、アプリ公開の準備に苦戦：初めてのモバイル開発経験であったため、App Store へのアプリ公開に際して注意すべき点が明確ではありませんでした。  
  ガイドラインの理解と、必要な準備作業について、独学で知識を深める必要がありました。

### プロジェクトの成果

- 元々感じていた課題を解決し、自分たちでも使っていきたいアプリを作ることができました。
- 初動は DAU:5 でしたが、改善を重ねるにつれ、10 倍/100 倍となっていきました。

#### 学びと成長

このプロジェクトを通じて、次のような学びと成長を得ました。

- 0 からモバイルアプリを作成したことで、ナレッジが溜まり、以降の案件でも積極的にモバイル開発を提案できるようになりました。
- 小さな改善でも積み重ねていくことで、利用してくれるユーザーが増えていくことを実感しました。
